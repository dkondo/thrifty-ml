...
def filter(
    ...
    llm: str = typer.Option("openai/gpt-4o-mini", "--llm"),
    embedding_model: str = typer.Option("openai/text-embedding-3-large", "--embedding-model"),
...
def embed(
    input: Path = typer.Argument(...),
    text_col: str = typer.Option(..., "--text-col", "-t"),
    model: str = typer.Option("openai/text-embedding-3-large", "--model"),
    out: Path = typer.Option(..., "--out", "-o"),
    cache_dir: Optional[Path] = typer.Option(None, "--cache-dir"),
) -> None:
